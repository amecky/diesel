...
float sdPryamid4(vec3 p, vec3 h) // h = { cos a, sin a, height }
{
	...
	float box = sdBox(p - vec3(0, -2.0*h.z, 0), vec3(2.0*h.z, 2.0*h.z, 2.0*h.z));
...
vec2 map(in vec3 pos)
{
	...
	res = opU(res, vec2(opS(udRoundBox(pos - vec3(-2.0, 0.2, 1.0), vec3(0.15, 0.15, 0.15), 0.05),
...
vec3 render(in vec3 ro, in vec3 rd)
{
	...
	if (m>-0.5)
	{
		...
		if (m<1.5)
		{
			...
			col = 0.3 + 0.1*f*vec3(1.0, 1.0, 1.0);
		}
...
float4 PS_Main(PS_Input frag) : SV_TARGET {
	...
	if (uv.x >= 0.0 && uv.y >= 0.0 && uv.x <= 1.0 && uv.y <= 1.0) {
		...
		// ray direction
		vec3 np = normalize(vec3(p.xy, 2.0));
		vec3 rd = mul(np, ca);
